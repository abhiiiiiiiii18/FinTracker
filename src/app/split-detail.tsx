...
function AddExpenseModal({
  ...
  const handleSave = async () => {
    ...
    if (desc.trim().length > 150) {
      Alert.alert("Invalid", "Description must be less than 150 characters.");
      return;
    }
    ...
    if (total > 1000000) {
      Alert.alert("Invalid", "Amount cannot exceed ₹10,00,000.");
      return;
    }
    ...
    if (splitType === "equal") {
      ...
      if (splits.some((s) => s.amount < 0)) {
        Alert.alert("Invalid", "Share amounts cannot be negative.");
        return;
      }
      ...
      if (Math.abs(sumShares - total) > 0.5) {
        Alert.alert(
          "Mismatch",
          `Shares sum (₹${sumShares.toFixed(2)}) must equal total (₹${total.toFixed(2)}).`,
        );
        return;
      }
    }
        ...
        <View style={styles.modalSheet}>
          ...
          >
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={reset} style={styles.modalCloseBtn}>
            ...
            <TouchableOpacity
              style={styles.picker}
              onPress={() => setShowPaidPicker(!showPaidPicker)}
              activeOpacity={0.8}
            >
              ...
              <View style={styles.pickerDropdown}>
                {members.map((m) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.pickerOption,
                      m === paidBy && styles.pickerOptionSelected,
                    ]}
                    onPress={() => {
                      setPaidBy(m);
            ...
            <View style={styles.splitToggle}>
              <TouchableOpacity
                style={[
                  styles.toggleOption,
                  splitType === "equal" && styles.toggleOptionActive,
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.toggleOption,
                  splitType === "custom" && styles.toggleOptionActive,
                ]}
...
function InviteMemberModal({
  ...
  const handleInvite = async () => {
    if (!name.trim()) {
      Alert.alert("Missing", "Enter a display name.");
      return;
    }
    if (!email.trim() || !email.includes("@")) {
      Alert.alert("Missing", "Enter a valid email.");
      return;
    }
    setLoading(true);
    ...
    Alert.alert(
      "✅ Invited!",
      `${name} will see this group when they log in with ${email}.`,
    );
    setEmail("");
    setName("");
    onClose();
        ...
        <View style={styles.modalSheet}>
          ...
          <View style={styles.inviteHintBox}>
            <Text style={styles.inviteHint}>
              They'll see this group and receive notifications instantly upon
              signup.
            </Text>
          </View>
...
function UPISettleModal({
  ...
  const openUPI = async () => {
    if (!upiId.trim()) {
      Alert.alert("Missing", "Enter the UPI ID to pay.");
      return;
    }
    ...
    if (!canOpen) {
      Alert.alert(
        "No UPI App",
        "Install GPay, PhonePe or any UPI app to proceed.",
      );
      return;
    }
    ...
    Alert.alert("Mark Settled?", "Did you complete the payment?", [
      { text: "Not yet", style: "cancel" },
      {
        text: "Yes, Settled!",
        onPress: () => {
          settleDebt(groupId, from, to, amount);
          onClose();
        },
      },
    ]);
        ...
        <View style={styles.modalSheet}>
          ...
          <View style={styles.upiCard}>
            <View style={styles.upiRow}>
              <View style={styles.upiAvatarWrap}>
                <Text style={styles.upiAvatar}>{from[0]}</Text>
              </View>
            ...
            <View style={styles.upiRow}>
              <View
                style={[
                  styles.upiAvatarWrap,
                  { backgroundColor: colors.violetGlow },
                ]}
              >
                <Text style={[styles.upiAvatar, { color: colors.violet }]}>
                  {to[0]}
                </Text>
              </View>
          ...
          >
            <Text style={styles.markSettledBtnText}>
              Mark as Settled (Skip UPI)
            </Text>
          </TouchableOpacity>
        </View>
      </View>
...
export default function SplitDetailScreen() {
  ...
  const [upiSettle, setUpiSettle] = useState<{
    from: string;
    to: string;
    amount: number;
  } | null>(null);
  ...
  if (!group) {
    return (
      <SafeAreaView style={styles.container}>
        <Text
          style={{ color: colors.text, textAlign: "center", marginTop: 80 }}
        >
          Group not found.
        </Text>
      </SafeAreaView>
  ...
  const handleShare = async () => {
    ...
    if (debts.length === 0) {
      ...
      debts.forEach((d) =>
        lines.push(`• ${d.from} owes ${d.to} ₹${d.amount.toFixed(0)}`),
      );
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <ScrollView
          ...
          style={styles.membersRowContainer}
        >
              ...
              return (
                <View key={i} style={styles.memberBadgeWrap}>
                  <View
                    style={[
                      styles.memberBadge,
                      isPending && styles.memberBadgePending,
                    ]}
                  >
              ...
              <View key={i} style={styles.debtCard}>
                <View style={styles.debtRow}>
                  <View style={styles.debtAvatar}>
                    <Text style={styles.debtAvatarText}>{d.from[0]}</Text>
                  </View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setUpiSettle({ from: d.from, to: d.to, amount: d.amount })
                  }
                  activeOpacity={0.8}
                >
                  <LinearGradient
        ...
        <View style={styles.section}>
          ...
          {sortedExpenses.length === 0 ? (
            <View style={styles.emptyExpBox}>
              ...
              <Text style={styles.emptyExpText}>
              ...
              return (
                <View key={exp.id} style={styles.expCard}>
                  <View
                    style={[
                      styles.expIconWrap,
                      isSettlement && { backgroundColor: colors.mintGlow },
                    ]}
                  >
                    <Text style={styles.expEmoji}>
                      {getExpenseEmoji(exp.description)}
                    </Text>
                  </View>
                  <View style={styles.expInfo}>
                    <Text style={styles.expDesc}>{exp.description}</Text>
                    <Text style={styles.expMeta}>
                      {isSettlement ? exp.paid_by : `${exp.paid_by} paid`} ·{" "}
                  ...
                  <Text
                    style={[
                      styles.expTotal,
                      isSettlement && { color: colors.mint },
                    ]}
                  >
