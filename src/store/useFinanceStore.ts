...
export type TransactionCategory = 'Food' | 'Transport' | 'Entertainment' | 'Bills' | 'Other';
...
export const useFinanceStore = create<FinanceState>()(
  (set, get) => ({
    ...
    budget: {
      totalLimit: 1000,
      categoryLimits: { Food: 300, Transport: 150, Entertainment: 100, Bills: 400, Other: 50 },
    },
    ...
    addTransaction: async (tx) => {
      ...
      set((state) => ({
         transactions: state.transactions.map(t => t.id === tempTx.id ? { ...t, id: data.id } : t)
      }));
    },
    ...
    syncSMS: async () => {
      try {
        ...
        for (const msg of messages) {
          ...
          if (parsed.externalId && localTransactions.some(t => t.external_id === parsed.externalId)) {
            continue;
          }
        ...
        if (txsToInsert.length > 0) {
          ...
          const { error } = await supabase.from('transactions').upsert(txsToInsert, { onConflict: 'external_id' });
    ...
    getInsights: () => {
      ...
      if (totalSpent > budget.totalLimit) {
        insights.push('🚨 You have exceeded your overall budget limit for the month!');
      } else if (totalSpent > budget.totalLimit * 0.8) {
        insights.push('⚠️ You are nearing your total budget limit. Time to cut back on spending.');
      }
      ...
      Object.entries(categorySpending).forEach(([cat, amount]) => {
        ...
        if (limit > 0 && amount > limit) {
          insights.push(`💡 Suggestion: You overspent on ${cat}. Try cooking at home or finding free alternatives this week.`);
        }
      });

      if (insights.length === 0) {
        insights.push('⭐️ Great job! You are keeping your expenses well within limits.');
      }
